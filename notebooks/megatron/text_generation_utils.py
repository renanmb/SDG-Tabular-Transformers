...
def synced_generate(model, context_tokens_tensor, context_length_tensor, tokens_to_generate, all_probs, temperature):
    ...
    if mpu.is_pipeline_last_stage():
        ...
        if mpu.is_pipeline_first_stage():
            src = mpu.get_pipeline_model_parallel_last_rank()
            group = mpu.get_embedding_group()
            output_logits = torch.empty(tokens.size(0), context_length - 1, dtype=torch.float32,
            ...
            if all_probs:
                ...
                src = mpu.get_pipeline_model_parallel_last_rank()
                group = mpu.get_embedding_group()
                full_logits = torch.empty(tokens.size(0), context_length, args.padded_vocab_size, dtype=torch.float32,
                                          device=torch.device("cuda"))
                torch.distributed.broadcast(full_logits, src, group)
        ...
        for decode_token in decode_tokens:
            ...
            if not isinstance(tokenizer, TabularTokenizer):
                ...
                for token in decode_token:
                    ...
                    word = bytearray([tokenizer.tokenizer.byte_decoder[c] for c in word]).decode('utf-8',
                                                                                                 errors='replace')
                    words.append(word)
...
def tab_sample_sequence_batch(model, context_tokens, context_lengths,
                              ...
                              tokens_to_generate, all_probs=False, type_ids=None, temperature=None):
    args = get_args()
    tokenizer: TabularTokenizer = get_tokenizer()
    ...
    with torch.no_grad():
        ...
        while context_length < maxlen:
            ...
            if mpu.is_pipeline_last_stage():
                ...
                output = output.float()
                logits = output[:, -1].view(batch_size, -1).contiguous()
                token_in_row = counter % tokens_per_row
                if args.greedy:
                    prev = torch.argmax(logits, dim=-1).view(-1)
                else:
                    logits = logits.float()
